...
var TabsOutlinerPane = function (tree) {
    ...
    this.update = function() {
        ...
        var walk = function (tree, domRoot) {
            console.log("XXX update.walk");
            if (tree.content === null) 
                tree.content = "root";

            // for each child of current node
            console.log("XXX for each child of "+ tree.content);
            for (var node in tree.children) {
                node = tree.children[node];
                console.log("XXX child: "+node);
                var liElt = that.pane.document.createElement("li");
                if (node.children.length == 0) {
                    liElt.innerHTML = "<span>"+node.content.title+"</span>";
                } else {
                    console.log("YYY collapsable");
        ...
        onTrack: function (win) {
            ...
            if (winutils.isBrowser(win)) {
                ...
                if (!(that.tree.has(win))) {
                    ...
                    that.tree.add(win);
                    console.log("XXX" + win.getBrowser().tabContainer.tabbrowser.tabs)
                    for (var index = 0; index < win.getBrowser().tabContainer.tabbrowser.tabs.length; index++) {
                        tab = win.getBrowser().tabContainer.tabbrowser.tabs[index];
