            ...
            for (var node in tree.children) {
                ...
                if (node.children.length == 0) {
                    ...
                    if (tree.children.indexOf(node) == tree.children.length-1) {
                        ...
                        liElt.innerHTML = "<div class='last hitarea collapsable-hitarea lastCollapsable-hitarea'></div><span class='" + spanClass + "'><img class='favicon' src='"+ icon +"' />"+getClass(node.content)+"</span>"; 
                    } else {
                        var icon = node.content.image ? node.content.image : self.data.url("images/firefox-small.png");
                        liElt.className = "collapsable" + liClass;
                        liElt.innerHTML = "<div class='hitarea collapsable-hitarea'></div><span class='" + spanClass + "'><img class='favicon' src='"+ icon +"' />"+getClass(node.content)+"</span>"; 
                    }
